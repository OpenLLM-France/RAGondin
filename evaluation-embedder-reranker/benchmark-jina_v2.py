...
async def retrieve_docs(entry, semaphore=asyncio.Semaphore(5)):
    async with semaphore:
        ...
        rankings = model.rank(question, list_file_content, return_documents=True, convert_to_tensor=True)
        ...
        list_chunks_id = [list_chunks_id[list_index[i]] for i in range(min(len(list_index), 5))]
        list_file_name = [list_file_name[list_index[i]] for i in range(min(len(list_index), 5))]
        # Get hit rate 
...
async def main():
    ...
    tasks = []
    for entry in tqdm(list_questions):
        try:
