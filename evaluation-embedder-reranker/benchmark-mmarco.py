...
from transformers import AutoTokenizer, AutoModelForSequenceClassification #ajouté
import torch
...
async def retrieve_docs(entry, semaphore=asyncio.Semaphore(2)):
    async with semaphore:
        ...
        features = tokenizer([(question, content) for content in list_file_content], 
                                padding=True,
                                truncation=True,
                                return_tensors="pt"
        )
...
async def main():
    out_file = "./ragondin/evaluation-embedder-reranker/complete_dataset_v2.json"
